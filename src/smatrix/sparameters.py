...
def _propagating_kz(eps_mu, Kx_norm_dn, Ky_norm_dn, evanscent, use_abs_out=False):
    """Return the real part of the normalized kz for the given medium.

    Evanescent orders (|real/imag| < evanscent) are masked to zero, or to
    |real| when use_abs_out is True, and the result is doubled along the
    x/y polarization axis.
    """

    Kz_norm_dn_complex = torch.sqrt(
        eps_mu - Kx_norm_dn**2 - Ky_norm_dn**2
    )
    is_evanescent = (
        torch.abs(torch.real(Kz_norm_dn_complex) / torch.imag(Kz_norm_dn_complex))
        < evanscent
    )
    if use_abs_out:
        Kz_norm_dn = torch.where(
            is_evanescent,
            torch.abs(torch.real(Kz_norm_dn_complex)),
            torch.real(Kz_norm_dn_complex),
        )
    else:
        Kz_norm_dn = torch.where(
            is_evanescent,
            torch.real(torch.zeros_like(Kz_norm_dn_complex)),
            torch.real(Kz_norm_dn_complex),
        )
    return torch.hstack((Kz_norm_dn, Kz_norm_dn))


def _doubled_real(x):
    """Repeat the real part of x along the x/y polarization axis."""

    return torch.hstack((torch.real(x), torch.real(x)))


def _S_parameters_xy(
    self,
    order_indices,
    ref_order_index,
    direction,
    ...
    if power_norm:
        Kz_norm_dn_in = _propagating_kz(
            self.eps_in * self.mu_in, self.Kx_norm_dn, self.Ky_norm_dn, evanscent
        )
        Kz_norm_dn_out = _propagating_kz(
            self.eps_out * self.mu_out, self.Kx_norm_dn, self.Ky_norm_dn, evanscent
        )

        Kx_norm_dn = _doubled_real(self.Kx_norm_dn)
        Ky_norm_dn = _doubled_real(self.Ky_norm_dn)
